...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for gene in &gene_models {
        println!("Gene: {} ({}:{}-{}) with {} transcripts", 
                 gene.id, gene.chromosome, gene.start, gene.end, gene.transcripts.len());
        
        for transcript in &gene.transcripts {
            println!("  Transcript: {} with {} exons, {} CDS regions", 
                     transcript.id, transcript.exons.len(), transcript.cds_regions.len());
        }
    }
