...
use crate::types::{
    AnnoRefineError, CdsRegion, Exon, FeatureType, GeneModel, Gff3Feature, Result, Strand,
    Transcript,
};
use log::{debug, info};
...
pub fn parse_gff3_file<P: AsRef<Path>>(path: P) -> Result<Vec<GeneModel>> {
    ...
    let file = File::open(path).map_err(|e| {
        AnnoRefineError::Gff3Parse(format!(
            "Failed to open GFF3 file {}: {}",
            path.display(),
            e
        ))
    })?;
...
fn parse_gff3_line(line: &str) -> Result<Gff3Feature> {
    ...
    if fields.len() != 9 {
        return Err(AnnoRefineError::Gff3Parse(format!(
    ...
    let start = fields[3].parse::<u64>().map_err(|_| {
        AnnoRefineError::Gff3Parse(format!("Invalid start position: {}", fields[3]))
    })?;

    let end = fields[4]
        .parse::<u64>()
        .map_err(|_| AnnoRefineError::Gff3Parse(format!("Invalid end position: {}", fields[4])))?;
    ...
    let score = if fields[5] == "." {
        ...
        Some(
            fields[5]
                .parse::<f64>()
                .map_err(|_| AnnoRefineError::Gff3Parse(format!("Invalid score: {}", fields[5])))?,
        )
    };
    ...
    let phase = if fields[7] == "." {
        ...
        Some(
            fields[7]
                .parse::<u8>()
                .map_err(|_| AnnoRefineError::Gff3Parse(format!("Invalid phase: {}", fields[7])))?,
        )
    };
    ...
    while let Some(c) = chars.next() {
        if c == '%' {
            ...
            let byte = u8::from_str_radix(&hex, 16).map_err(|_| {
                AnnoRefineError::Gff3Parse(format!("Invalid hex in URL encoding: {}", hex))
            })?;
...
fn build_gene_models(features: Vec<Gff3Feature>) -> Result<Vec<GeneModel>> {
    ...
    debug!(
        "Found {} genes, {} transcripts, {} exons, {} CDS regions",
        genes.len(),
        transcripts.len(),
        exons.len(),
        cds_regions.len()
        ...
        for (transcript_id, transcript_feature) in &transcripts {
            if let Some(parent) = transcript_feature.get_parent() {
                if parent == gene_id {
                    // Build transcript
                    let transcript =
                        build_transcript(transcript_id, transcript_feature, &exons, &cds_regions)?;
