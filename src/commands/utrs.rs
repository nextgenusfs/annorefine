...
use annorefine::{
    ...
    types::{LibraryType, RefinementConfig, StrandBias, AnnoRefineError},
};
...
pub struct UtrsCommand {
    ...
    #[arg(long = "strand-bias-threshold", value_name = "THRESHOLD", default_value = "0.65")]
    ...
    #[arg(long = "max-reads-strand-detection", value_name = "N", default_value = "10000")]
    ...
    pub fn run(self) -> Result<()> {
        ...
        if let Some(threads) = self.threads {
            rayon::ThreadPoolBuilder::new()
                .num_threads(threads)
                .build_global()
                .map_err(|e| AnnoRefineError::InvalidInput(format!("Failed to set thread count: {}", e)))?;
        ...
        for file in [&self.fasta_file, &self.gff3_file, &self.bam_file] {
            if !file.exists() {
                return Err(AnnoRefineError::InvalidInput(format!("File not found: {}", file.display())).into());
            }
        }
        ...
        let library_type: LibraryType = self.stranded.parse()
            .map_err(|e| AnnoRefineError::InvalidInput(format!("Invalid --stranded option: {}", e)))?;
