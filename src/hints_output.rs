...
use crate::types::{AugustusHint, HintType, Result, AnnoRefineError};
use std::io::Write;
use log::debug;
...
impl<W: Write> HintsWriter<W> {
    ...
    pub fn write_header(&mut self, input_file: &str, config_summary: &str) -> Result<()> {
        writeln!(self.writer, "# gene model hints generated by annorefine bam2hints")?;
        writeln!(self.writer, "# Input BAM file: {}", input_file)?;
        writeln!(self.writer, "# Configuration: {}", config_summary)?;
        writeln!(self.writer, "# Format: seqname\tsource\tfeature\tstart\tend\tscore\tstrand\tframe\tattributes")?;
        Ok(())
    }
    ...
    pub fn write_hints_sorted_by_chromosome_order(&mut self, mut hints: Vec<AugustusHint>, chromosome_order: &[String]) -> Result<()> {
    ...
    fn apply_coverage_filter(
        ...
        debug!("Coverage filtering: {} -> {} hints", hints_len, filtered.len());
        Ok(filtered)
    }
...
pub mod utils {
    ...
    pub fn remove_redundant_hints(hints: Vec<AugustusHint>) -> Vec<AugustusHint> {
        ...
        debug!("Redundancy removal: {} -> {} hints", hints_len, filtered.len());
        filtered
    }
    ...
    pub fn merge_overlapping_hints(hints: Vec<AugustusHint>) -> Vec<AugustusHint> {
        ...
        use std::collections::HashMap;
        let mut groups: HashMap<(String, HintType, crate::types::Strand), Vec<AugustusHint>> = HashMap::new();
