...
use crate::types::{Transcript, Genome, Strand, Result};
...
fn validate_cds_sequence(
    ...
    if !result.has_start_codon {
        result.issues.push(format!("Invalid start codon: {}", start_codon));
    }
    ...
    if !result.has_stop_codon && cds_string.len() >= 3 {
        ...
        if !genetic_code.is_stop_codon(last_codon) {
            result.issues.push("Missing stop codon at end of CDS".to_string());
        }
    }
    ...
    result.is_valid = result.has_start_codon && 
                     ...
                     result.issues.iter().all(|issue| 
                         issue.contains("non-coding") || 
                         issue.starts_with("Missing stop codon")
                     );
...
pub fn validate_cds_after_refinement(
    ...
    let validation_result = validate_cds(refined_transcript, genome, chromosome, strand, genetic_code)?;
    
    if !validation_result.is_valid {
        debug!("CDS validation failed for transcript {}: {:?}", 
    ...
    for cds_region in &refined_transcript.cds_regions {
        ...
        if !cds_covered {
            debug!("CDS region {}-{} not covered by any exon in transcript {}", 
...
pub fn update_cds_after_exon_changes(
    ...
    for cds_region in &transcript.cds_regions {
        ...
        if cds_valid {
            ...
            warn!("Removing CDS region {}-{} that is no longer within exon boundaries", 
